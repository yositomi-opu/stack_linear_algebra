(() => {
  ...
  const translations = new Map(Object.entries({
    ...
    "問題文に __SELPROMPT__ を入れると、上の選択肢設定に応じて「正しいものを1つ選べ。」「正しいものをすべて選べ。」などの指示文が自動的に入ります。": "Add __SELPROMPT__ to the question text to automatically insert instructions such as ‘Select one correct option.’ or ‘Select all correct options.’ according to the option settings above.",
    "Radio形式で正解が2つ以上になり得る場合は、「正しいものを1つ選べ（複数ある場合も1つでよい）。」という説明まで入ります。": "For Radio questions that may have multiple correct options, the instructions also explain that selecting just one correct option is enough.",
    "自動の指示文が気に入らない場合は、__SELPROMPT__ を使わずに、自分で好きな指示文を書いても問題ありません。": "If you prefer different wording, you can leave out __SELPROMPT__ and write your own instructions.",
    ...
    "真偽ごとにフィードバックを変える": "Use different feedback for true and false options",
    "追加時は真偽別フィードバック": "Use separate true/false feedback for new patterns",
    "このパターンは真偽別": "Separate true/false feedback for this pattern",
    "選択肢データ": "Option data",
    "行追加": "Add row",
    "クリア": "Clear",
    "パターン": "Pattern",
    ...
    "真偽別FB ja": "True/false feedback ja",
    ...
    "フィードバックは言語に依存しない": "Feedback is language-independent",
    "パターンを削除": "Delete pattern",
    "削除": "Delete",
    "フィードバックは同じパターンの先頭行で編集します": "Edit feedback in the first row of the same pattern",
    "フィードバックは同じパターン・真偽の先頭行で編集します": "Edit feedback in the first row with the same pattern and truth value",
    ...
    "真偽1対の選択肢行が追加されました": "A true/false option pair was added",
  ...
  const rules = [
    ...
    [/^選択肢 (\w+)$/, "Option $1"], [/^共通FB (\w+)$/, "Shared feedback $1"], [/^真偽別FB (\w+)$/, "True/false feedback $1"],
